...
+ (CCCryptorRef)createAESCTRCryptor:(CCOperation)op key:(NSData *)key iv:(NSData *)iv {
    CCCryptorRef cryptor = NULL;
    CCCryptorStatus status = CCCryptorCreateWithMode(op, kCCModeCTR, kCCAlgorithmAES, ccNoPadding, iv.bytes, key.bytes, key.length, NULL, 0, 0, kCCModeOptionCTR_BE, &cryptor);
    if (status != kCCSuccess) {
...
+ (NSData *)cryptorUpdate:(CCCryptorRef)cryptor data:(NSData *)data {
    if (!cryptor || data.length == 0) {
        return data;
    }
    size_t outLen = 0;
    void *outBuf = malloc(data.length);
    CCCryptorStatus status = CCCryptorUpdate(cryptor, data.bytes, data.length, outBuf, data.length, &outLen);
    if (status != kCCSuccess) {
        NSLog(@"BlufiSecurity cryptorUpdate error: %@", @(status));
        free(outBuf);
        return nil;
    }
...
+ (BlufiDH *)dhGenerateKeysWithP:(const Byte *)pBytes pLength:(size_t)pLength {
    DH *dh;
    int ret = 0, i;
    dh = DH_new();

    BIGNUM *dh_p = BN_bin2bn(pBytes, (int)pLength, NULL);
    BIGNUM *dh_g = BN_new();
    ...
    NSData *p = [NSData dataWithBytes:pBytes length:pLength];
    Byte gBuf[] = {DH_G};
    NSData *g = [NSData dataWithBytes:gBuf length:1];

    return [[BlufiDH alloc] initWithP:p G:g PublicKey:publicKey PrivateKey:privateKey DH:dh keySize:keySize];
}

+ (BlufiDH *)dhGenerateKeys {
    return [self dhGenerateKeysWithP:DH_P pLength:sizeof(DH_P)];
}

+ (BlufiDH *)dhGenerateKeys3072 {
    return [self dhGenerateKeysWithP:DH_P_3072 pLength:sizeof(DH_P_3072)];
}
@end
